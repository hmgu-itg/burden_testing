...
## Description:
# This script was written to automatically update the genomic region collection
...
function usage {
    ...
    echo ""
    echo ""
    echo "The output is a bed file, where the first 4 columns are the chromosome, start/end coordinates and the stable ID of the gene respectively. The 5th column is a json formatted string describing one genomic region associated to the given gene. This line contains all information of the association."
    echo ""
    echo "Tags:"
...
##
## Step 3. Downloading Ensembl Regulation (newest version, GRCh38 build)
##
...
Ensembl_release=84 # The Ensembl release versin is hardcoded
...
echo "[Warning] GTEx version is hardcoded! Please check if this is the most recent!\n"
...
for cell in ${CellTypes}; do
    export cell
    # parsing cell specific files (At this point we only consider active features. Although repressed regions can also be informative.):
    zcat ${targetDir}/${today}/EnsemblRegulation/RegulatoryFeatures_${cell}.gff.gz | grep "=active" \
...
' | sort -k1,1 -k2,2n | bgzip > ${targetDir}/${today}/processed/Cell_spec_regulatory_features_regress.bed.gz
...
# Print out report:
cellSpecFeatLines=$(zcat ${targetDir}/${today}/processed/Cell_spec_regulatory_features_regress.bed.gz | wc -l | awk '{print $1}')
...
intersectBed -wb -a ${targetDir}/${today}/processed/GTEx_temp_GRCh38.bed -b ${targetDir}/${today}/processed/Cell_spec_regulatory_features_regress.bed.gz \
    ...
    ' | gzip > ${targetDir}/${today}/processed/GTEx_Regulation_linked_regress.txt.gz

# Testing if output file has lines:
testFileLines ${targetDir}/${today}/processed/GTEx_Regulation_linked_regress.txt.gz
...
intersectBed -wb -a ${targetDir}/${today}/processed/genes.bed.gz -b ${targetDir}/${today}/processed/Cell_spec_regulatory_features_regress.bed.gz -sorted \
    | perl -MData::Dumper -MJSON -F"\t" -lane '
        # Parsing gene info:
        ($g_ID) = $F[3] =~ /ID:(ENSG\d+)/;
        ($g_name) = $F[3] =~ /Name:(.+)/;
    ...
    ' | bgzip > ${targetDir}/${today}/processed/overlapping_features_regress.txt.gz
echo "Done."
...
zcat ${targetDir}/${today}/processed/overlapping_features_regress.txt.gz \
     ${targetDir}/${today}/processed/GTEx_Regulation_linked_regress.txt.gz \
...
#
# GENCODE version: v.25
# Ensembl version: v.84
# GTEx version: 1.6
#

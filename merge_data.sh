...
genes=$(zcat ${targetDir}/GENCODE/gencode.v${GENCODE_release}.annotation.gtf.gz | awk 'BEGIN{FS="\t";}$3 == "gene"{print $3;}' | wc -l )
info "Total number of genes in the GENCODE file: ${genes}\n\n"

#=================================== REGULATION ==============================================

# Printing out report of the downloaded cell types:
cellTypeCount=$(ls -la ${targetDir}/EnsemblRegulation/*gff.gz | wc -l)
...
mkdir -p ${targetDir}/processed
export APPRIS_FILE=${targetDir}/APPRIS/appris_data.principal.txt
zcat ${targetDir}/GENCODE/gencode.v${GENCODE_release}.annotation.gtf.gz | grep -v "#" | awk '$3 != "Selenocysteine" && $3 != "start_codon" && $3 != "stop_codon"' \
			...
			$exonID=$1;
                ...
                }' | gzip > ${targetDir}/processed/Appris_annotation_added.txt.gz

# Test if output is empty:
testFile ${targetDir}/processed/Appris_annotation_added.txt.gz
testFileLines ${targetDir}/processed/Appris_annotation_added.txt.gz # 0-based
...
# Print out report:
appris_lines=$(zcat ${targetDir}/processed/Appris_annotation_added.txt.gz | wc -l | awk '{print $1}')
...
CellTypes=$( ls -la ${targetDir}/EnsemblRegulation/ | perl -lane 'print $1 if  $F[-1] =~ /(.+).gff.gz/ ' )
for cell in ${CellTypes}; do
    export cell
    fn=${targetDir}/EnsemblRegulation/${cell}.gff.gz
...
' | sort -k1,1 -k2,2n | bgzip -f > ${targetDir}/processed/Cell_spec_regulatory_features.bed.gz # 0-based coordinates here
...
testFileLines ${targetDir}/processed/Cell_spec_regulatory_features.bed.gz
tabix -f -p bed ${targetDir}/processed/Cell_spec_regulatory_features.bed.gz
echo  "Done."

# Print out report:
cellSpecFeatLines=$(zcat ${targetDir}/processed/Cell_spec_regulatory_features.bed.gz | wc -l | awk '{print $1}')
...
tmpGTEx=${targetDir}/processed/GTEx_tmp.bed
...
intersectBed -wb -a ${targetDir}/processed/GTEx.bed -b ${targetDir}/processed/Cell_spec_regulatory_features.bed.gz 2>/dev/null | perl -MData::Dumper -MJSON -F"\t" -lane '
        $source= "GTEx";
    ...
    ' | gzip > ${targetDir}/processed/GTEx_Regulation_linked.txt.gz
...
testFileLines ${targetDir}/processed/GTEx_Regulation_linked.txt.gz # start/end are 0-based
echo "Done"

# Generate report:
GTExLinkedFeatures=$( zcat ${targetDir}/processed/GTEx_Regulation_linked.txt.gz | wc -l | awk '{print $1}')
...
# generating a file.
# 0-based
zcat ${targetDir}/GENCODE/gencode.v${GENCODE_release}.annotation.gtf.gz | awk '$3 == "gene"' | perl -lane '
     $g_name="NA";
        if ($_ =~ /gene_name\s+\"(.+?)\";/){
	$g_name=$1;
	}
    ...
    ' | sort -k1,1 -k2,2n | bgzip -f > ${targetDir}/processed/genes.bed.gz # 0-based
...
intersectBed -wb -a ${targetDir}/processed/genes.bed.gz -b ${targetDir}/processed/Cell_spec_regulatory_features.bed.gz -sorted 2>/dev/null | perl -MData::Dumper -MJSON -F"\t" -lane '
        # Parsing gene info:
        ($g_ID) = $F[3] =~ /ID:(ENSG[^;]+);/;
        ($g_name) = $F[3] =~ /Name:(\S+)/;
    ...
    ' | bgzip -f > ${targetDir}/processed/overlapping_features.txt.gz # 0-based coordinates
echo "Done"
...
# Generate report:
OverlapLinkedFeatures=$( zcat ${targetDir}/processed/overlapping_features.txt.gz | wc -l | awk '{print $1}')
...
export gene_file=${targetDir}/processed/genes.bed.gz
...
zcat ${targetDir}/processed/overlapping_features.txt.gz \
     ${targetDir}/processed/GTEx_Regulation_linked.txt.gz \
     ${targetDir}/processed/Appris_annotation_added.txt.gz  \
     | perl -lane 'BEGIN {
        ...
        }'  2> ${targetDir}/failed | sort -k1,1 -k2,2n > ${targetDir}/Linked_features.bed # 0-based
...
#
# CHR\tSTART\tEND\tGENEID\tANNOTATION" ) ${targetDir}/Linked_features.bed | sponge ${targetDir}/Linked_features.bed

# Compressing and indexing output file:
bgzip -f ${targetDir}/Linked_features.bed > ${targetDir}/Linked_features.bed.gz
tabix -f -p bed ${targetDir}/Linked_features.bed.gz

info "Output file was saved as: Linked_features.bed.gz\n"
totalLines=$(zcat ${targetDir}/Linked_features.bed.gz | wc -l | awk '{print $1}')
info "Total number of lines in the final file: ${totalLines}\n"

# FOR LATER USE
mv -f ${targetDir}/Linked_features.bed.gz ${outdir}
mv -f ${targetDir}/Linked_features.bed.gz.tbi ${outdir}
...
zcat  ${targetDir}/GENCODE/gencode.v${GENCODE_release}.annotation.gtf.gz | grep -v "^#"| perl -F"\t" -lane 'next if $F[2] ne "gene";$x=$F[8];$id="NA";$id=$1 if ($x=~/gene_id\s+\"(ENSG[^"]+)\"/); $gn="NA"; $gn=$1 if $x=~/gene_name\s+\"([^"]+)\"/;$,="\t";$F[0]=~s/^chr//;print $F[0],$F[3],$F[4],$gn,$id;' | gzip > ${outdir}/gencode.basic.annotation.tsv.gz
zcat  ${targetDir}/GENCODE/gencode.v${GENCODE_release}.annotation.gtf.gz | grep -v "^#"| perl -F"\t" -lane 'next if $F[2] ne "gene";$x=$F[8];next unless $x=~/gene_type\s+\"protein_coding\"/;$id="NA";$id=$1 if ($x=~/gene_id\s+\"(ENSG[^"]+)\"/); $gn="NA"; $gn=$1 if $x=~/gene_name\s+\"([^"]+)\"/;$,="\t";$F[0]=~s/^chr//;print $F[0],$F[3],$F[4],$gn,$id;' | gzip > ${outdir}/gencode.basic.annotation.protein_coding.tsv.gz
...
FailedAssoc=$(wc -l ${targetDir}/failed | awk '{print $1}')
FailedGenes=$( cat ${targetDir}/failed | perl -lane '$_ =~ /(ENSG\d+)/; print $1' | sort | uniq | wc -l )
FailedSources=$( cat ${targetDir}/failed | perl -lane '$_ =~ /"source":"(.+?)"/; print $1' | sort | uniq | tr "\n" ", " )
...
if [[ $backup == "yes" ]];then
    ...
    tar czf ${targetDir}/${today}_annotation.backup.tar.gz --remove-file ${targetDir}/APPRIS ${targetDir}/EnsemblRegulation ${targetDir}/failed ${targetDir}/GENCODE ${targetDir}/processed
    info "Intermediate files are saved in ${today}_annotation.backup.tar.gz\n"
else
    rm -rf ${targetDir}
fi

if [[ -f "${outdir}/scores/eigen.phred_v2.dat" ]];then
    echo "EigenPath=${outdir}/scores/eigen.phred_v2.dat" >> ${configfile}
fi
if [[ -f "${outdir}/scores/whole_genome_SNVs.tsv.gz" ]];then

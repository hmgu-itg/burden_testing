...
find $workingDir -maxdepth 1 -type d | grep Pheno | while read folder; do
    echo -n "."
    ...
    cat <(echo -e "GeneName\tp-value\tSNP_count") \
        ...
        | gzip > ${workingDir}/summaries/Pooled_results.Pheno.${trait}.tsv
...
for file in $( find ${workingDir} | grep hits | grep gz) ; do
    echo -n "."

...
tabix $lfeatures $chr:$start-$end | grep "\"gene_ID\":\"$gene\""| cut -f 5| perl -MJSON `BEGIN{$extend=$ENV{extend};}{%h = %{decode_json($line)};if ($h{"source"} eq "GENCODE" && $h{"class"} eq "exon"){$,="\t";print $h{"chr"},$h{"start"}-$extend,$h{"end"}+$extend;}}` | sort -k1,1 -k2,2n > 01.regions.bed

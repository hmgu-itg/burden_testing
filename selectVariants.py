...
input_options.add_argument('--shift','-shift',action="store",type=float,help="Optional: shift scores by this value",default=config.DEFAULT_SHIFT,required=False)
input_options.add_argument('--cutoff','-cutoff',action="store",type=float,help="Optional: variants with scores below this threshold will not be included",default=config.DEFAULT_CUTOFF,required=False)
input_options.add_argument('--floor','-floor',action="store",type=float,help="Optional: scores will be set to this value if they fall below it",default=config.DEFAULT_FLOOR,required=False)
